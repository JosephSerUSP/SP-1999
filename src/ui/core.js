...
class Window_Base {
    ...
    refresh() {
        ...
        // Configure root
        if (blueprint) {
            if (blueprint.layout) this.root.update({ layout: blueprint.layout });
            if (blueprint.props) this.root.update(blueprint.props);

            // Build children
            if (blueprint.children) {
                blueprint.children.forEach(childDef => this.buildFromBlueprint(this.root, childDef));
            }
        }
    }
    ...
    buildFromBlueprint(parentContainer, def) {
        ...
        if (def.type === 'container') {
            ...
            // Check for children if the component supports adding them
            if (def.children && typeof component.add === 'function') {
                def.children.forEach(childDef => this.buildFromBlueprint(component, childDef));
            }
        }
