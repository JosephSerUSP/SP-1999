...
export class Renderer3D {
    ...
    /**
     * Attaches game state objects to the renderer.
     * @param {Game_Map} gameMap
     * @param {Game_Party} gameParty
     * @param {Game_System} gameSystem
     */
    attachGameState(gameMap, gameParty, gameSystem) {
    ...
    rebuildLevel() {
        ...
        this.mapGroup.clear();
        const count = this.gameMap.width * this.gameMap.height;
        ...
        for(let x=0; x<this.gameMap.width; x++) {
            for(let y=0; y<this.gameMap.height; y++) {
                if(this.gameMap.tiles[x][y] === 0 || this.gameMap.tiles[x][y] === 3) {
                ...
                } else if (this.gameMap.tiles[x][y] === 1) {
        ...
        this.playerMesh.position.set(this.gameMap.playerX, 0.5, this.gameMap.playerY);
        this.playerTarget.set(this.gameMap.playerX, 0.5, this.gameMap.playerY);
        this.cameraLookCurrent.set(this.gameMap.playerX, 0, this.gameMap.playerY);
    ...
    hasFloorNeighbor(x,y) {
        const t = this.gameMap.tiles;
        const check = (nx, ny) => nx>=0 && nx<this.gameMap.width && ny>=0 && ny<this.gameMap.height && t[nx][ny] !== 1;
    ...
    syncEnemies() {
        if (!this.gameMap) return;
        const cIds = new Set(this.gameMap.enemies.map(e => e.uid));
        ...
        this.gameMap.enemies.forEach(e => {
    ...
    /**
     * Applies events from the system layer.
    ...
    applyEvents(events) {
        ...
        events.forEach(e => {
            switch(e.type) {
                case 'playerMove':
                    this.playAnimation('move_switch', e);
                    break;
                case 'playerAttack':
                    this.playAnimation('lunge', e);
                    break;
                case 'enemyAttack':
                    this.playAnimation('enemyLunge', e);
                    break;
                case 'hitEffect':
                    this.playAnimation('hit', e);
                    break;
                case 'shake':
                    this.playAnimation('shake');
                    break;
                case 'itemGet':
                    this.playAnimation('itemGet', e);
                    break;
                case 'enemyDie':
                    this.playAnimation('die', e);
                    break;
                case 'syncEnemies':
                    this.syncEnemies();
                    break;
                case 'syncLoot':
                    ...
                    break;
                case 'ascend':
                    this.playAnimation('ascend');
                    break;
                case 'flash':
                    this.playAnimation('flash', e);
                    break;
                case 'projectile':
                    this.playAnimation('projectile', e);
                    break;
                case 'mapSetup':
                    this.rebuildLevel();
                    break;
    ...
    playAnimation(type, data) {
        if (type === 'move_switch') {
            ...
            // Visualize projectile - basic implementation using particles along the path
            const dist = Math.sqrt((data.x2-data.x1)**2 + (data.y2-data.y1)**2);
            const steps = 10;
            for(let i=0; i<=steps; i++) {
                const t = i/steps;
                this.particles.spawnBurst(data.x1 + (data.x2-data.x1)*t, data.y1 + (data.y2-data.y1)*t, data.color, 2);
            }
        }
    }
    ...
    projectToScreen(x, y, z) {
        ...
        return { x: (vec.x * width/2) + width/2, y: -(vec.y * height/2) + height/2 };
    }
    ...
    showRange(skill) {
        ...
        if (!skill || !this.gameMap) return;
        const geo = new THREE.PlaneGeometry(0.9, 0.9);
        const mat = new THREE.MeshBasicMaterial({ color: 0xff0000, transparent: true, opacity: 0.3, side: THREE.DoubleSide });
        const px = this.gameMap.playerX; const py = this.gameMap.playerY; const coords = [];
        if (skill.type === 'all_enemies') { this.gameMap.enemies.forEach(e => coords.push({x: e.x, y: e.y})); }
        else if (skill.range > 0) {
            for(let x = px - skill.range; x <= px + skill.range; x++) {
                for(let y = py - skill.range; y <= py + skill.range; y++) {
                    if (Math.abs(x - px) + Math.abs(y - py) <= skill.range && this.gameMap.isValid(x, y) && this.gameMap.tiles[x][y] === 0) coords.push({x, y});
                }
            }
        }
        coords.forEach(c => {
     ...
     */
    clearRange() { if (this.rangeGroup) this.rangeGroup.clear(); }
