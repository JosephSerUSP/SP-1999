...
export class UIManager {
     ...
     * @param {Game_System} gameSystem
     * @param {Game_Party} gameParty
     * @param {Game_Map} gameMap
     * @param {BattleManager} battleManager (Not strictly needed as a dependency if we pass commands via callbacks, but useful)
     * @param {ItemManager} itemManager (Same)
     * @param {CutsceneManager} cutsceneManager
     * @param {Renderer3D} renderer
     * @param {Function} onAction - Callback for actions like moving or using skills.
     */
    constructor(gameSystem, gameParty, gameMap, battleManager, itemManager, cutsceneManager, renderer, onAction) {
        ...
        // Subscribe to logs
        this.gameSystem.onLog = () => this.refreshLog();
    }
    ...
    createLayout() {
        ...
        this.windows.minimap.content.appendChild(cvs); this.windows.minimap.content.style.padding = '0';
    ...
    refreshMinimap() {
        ...
        const mapW = this.gameMap.width, mapH = this.gameMap.height;
        const tw = w / mapW, th = h / mapH;
        ctx.fillStyle = "#000"; ctx.fillRect(0,0,w,h);
        for(let x=0; x<mapW; x++) {
            for(let y=0; y<mapH; y++) {
                if(!this.gameMap.tiles[x]) continue;
                const t = this.gameMap.tiles[x][y];
                if(t === 1) { ctx.fillStyle = "#444"; ctx.fillRect(x*tw, y*th, tw, th); }
                else if(t === 3) { ctx.fillStyle = "#0f0"; ctx.fillRect(x*tw, y*th, tw, th); }
            }
        }
        this.gameMap.enemies.forEach(e => { ctx.fillStyle = "#f00"; ctx.fillRect(e.x*tw, e.y*th, tw, th); });
        ctx.fillStyle = "#0ff"; ctx.fillRect(this.gameMap.playerX*tw, this.gameMap.playerY*th, tw, th);
    }
    ...
    refreshStatus() {
        ...
        this.gameParty.members.forEach((m, i) => {
            const active = i === this.gameParty.index;
            const div = document.createElement('div'); div.className = `party-slot ${active ? 'active' : ''}`;
            div.onclick = () => { if(!this.gameSystem.isBusy && !this.gameSystem.isInputBlocked) this.showStatusModal(m); };
    ...
    refreshCmd() {
        ...
        const btn = (t, s, cb, dis, sk) => {
            ...
            b.onclick = () => { if(!this.gameSystem.isBusy && !this.gameSystem.isInputBlocked && !dis) cb(); };
            if(sk) {
                b.onmouseenter = (e) => { this.renderer.showRange(sk); this.showTooltip(e, `<b>${sk.name}</b><br>${sk.desc(actor)}`); };
                b.onmouseleave = () => { this.renderer.clearRange(); this.hideTooltip(); };
            }
            w.content.appendChild(b);
        };
        btn("WAIT", "", () => this.onAction('wait'));
        ...
        actor.skills.forEach(k => {
            ...
            btn(s.name, `${s.cost}PE`, () => {
                // If the skill requires a turn (most do), we process it via onAction
                // However, the original code did: $gameMap.processTurn(0,0,()=>BattleManager.executeSkill(actor, k));
                // We will simplify: The main loop calls processTurn, and if a skill is used, it's part of the action.
                this.onAction('skill', k);
            }, actor.pe < s.cost, s);
        });
    }
    ...
    refreshLog() {
        ...
        this.gameSystem.logHistory.slice(0,lines).forEach((t,i) => {
     ...
     */
    floatText(t, x, y, c) {
        const pos = this.renderer.projectToScreen(x, 0.5, y);
    ...
    showInventoryModal() {
        ...
        const render = (c) => {
            c.innerHTML = '';
            c.style.display = "flex"; c.style.gap = "10px"; c.style.height = "100%";
            ...
            if(this.gameParty.inventory.length === 0) leftCol.innerHTML = "<div style='color:#666; text-align:center; padding-top:20px;'>Empty</div>";
            this.gameParty.inventory.forEach((i, idx) => {
            ...
            this.gameParty.members.forEach(m => {
                ...
                const renderEquip = (slot, item) => {
                    ...
                    if(item) {
                        ...
                        row.onclick = () => {
                            if(this.gameParty.inventory.length < this.gameParty.maxInventory) {
                                this.gameParty.gainItem(item); m.equip[slot] = null; render(c); this.refresh();
        ...
        this.createModal(`SHARED INVENTORY [${this.gameParty.inventory.length}/${this.gameParty.maxInventory}]`, render, () => this.gameSystem.isInputBlocked=false);
    }
    ...
    useConsumable(actor, i, idx) {
        if(i.type==='heal') { actor.heal(i.val); this.gameSystem.log(`Healed ${actor.name} for ${i.val}.`); }
        if(i.type==='pe') { actor.pe = Math.min(actor.mpe, actor.pe+i.val); this.gameSystem.log(`Restored PE for ${actor.name}.`); }
        this.gameParty.inventory.splice(idx, 1);
        this.closeModal();
        this.gameSystem.isInputBlocked = false;
        this.refresh();
        this.onAction('wait'); // Consuming an item takes a turn
    }
    ...
    equipGear(actor, i, idx, cb) {
        ...
        actor.equip[type] = i; this.gameParty.inventory.splice(idx, 1);
        if(current) this.gameParty.gainItem(current);
        this.gameSystem.log(`${actor.name} equipped ${i.name}.`);
        cb();
    }
    ...
    /**
     * Handles cutscene dialog events.
     * @param {Object} e - Event data.
     */
    handleCutsceneEvent(e) {
         if(e.type === 'dialog') {
             const dialogEl = document.getElementById('cutscene-overlay');
             dialogEl.innerHTML = `
                <div style="border-bottom:1px solid #444; margin-bottom:5px; padding-bottom:2px; font-weight:bold; color:${e.color || '#0ff'}">${e.speaker || 'SYSTEM'}</div>
                <div style="font-size:14px; margin-bottom:10px;">${e.text}</div>
